...
def create_table(dbname, table_name, structure):
    ...
    db = sql.connect(host="localhost", user="root", password="1234", database=dbname)
...
def backup_db(dbname, path, differential):
    ...
    if differential == 0:
        cursor.execute("BACKUP DATABASE %s TO DISK = '%s'" %(dbname, path))
    else:
        cursor.execute("BACKUP DATABASE %s TO DISK = '%s' WITH DIFFERENTIAL" %(dbname, path))
    
    db.commit()
    db.close()

def restore_db(dbname, path):

    db = sql.connect(host="localhost", user="root", password="1234")

    cursor = db.cursor()

    cursor.execute("RESTORE DATABASE %s FROM DISK = '%s'" %(dbname, path))
